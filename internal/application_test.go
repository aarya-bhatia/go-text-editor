...
func TestApplication_SingleFile(t *testing.T) {
  ...
  app.OpenFile(f.Name())
	...
	assert.True(t, app.CurrentFile.Name == f.Name())
...
func TestApplication_MultipleFiles(t *testing.T) {
	f1, _ := os.CreateTemp("", TEMP_FILE+".0")
	f2, _ := os.CreateTemp("", TEMP_FILE+".1")
	f3, _ := os.CreateTemp("", TEMP_FILE+".2")
